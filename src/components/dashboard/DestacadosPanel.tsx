...
interface DestacadosPanelProps {
  alumnos: AlumnoConProgreso[]
}

export function DestacadosPanel({ alumnos }: DestacadosPanelProps) {
  return (
      ...
      {alumnos.length === 0 ? (
        ...
        <div className="space-y-3">
          {alumnos.map((a, i) => (
            ...
            >
              <div className="text-sm text-slate-500 font-mono w-5 text-right shrink-0">
                {i + 1}
              </div>
              <div
              ...
              <span className="text-emerald-400 text-xs font-bold shrink-0">{Math.round(a.progreso)}%</span>
            </motion.div>
          ))}
        </div>
      )}
